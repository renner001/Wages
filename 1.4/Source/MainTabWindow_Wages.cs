...
namespace DanielRenner.Wages
{
    public class MainTabWindow_Wages : MainTabWindow
    {
        ...
        public override void DoWindowContents(Rect canvas)
        {
            ...
            Widgets.Label(topRowSlider, "Wages (paid every five days in silver)");
            ...
            /*if (ModSettings_Wages.fixedIncome)
            {
                ...
                for (int i = 1; i < moodSteps.Length; i++)
                {
            ...
            foreach (var pawn in allPawns)
            {
                ...
                var nameRect = nameAndValueRect.LeftPartPixels(nameAndValueRect.width - valuesWidth);
                Widgets.Label(nameRect, pawn.NameFullColored);

                var valuesRect = nameAndValueRect.RightPartPixels(valuesWidth);
                var valuesSilverRect = valuesRect.TopHalf();
                var valuesSilverIconRect = valuesSilverRect.LeftPartPixels(Text.LineHeight);
                Widgets.ThingIcon(valuesSilverIconRect, ThingDefOf.Silver);
                valuesSilverRect = valuesSilverRect.RightPartPixels(valuesSilverRect.width - Text.LineHeight);
                Widgets.Label(valuesSilverRect, wageSettingsPerPawn[pawn] + " silver");
                ...
                valuesMoodRect = valuesMoodRect.RightPartPixels(valuesSilverRect.width - Text.LineHeight);
                var moodValue = PayHelperUtility.MoodFromWage(pawn, wageSettingsPerPawn[pawn]);
                var moodText = "";
                if (moodValue >= 0)
                {
                    moodText = "+" + moodValue;
                ...
                {
                    moodText = moodValue.ToString();
                }
                Widgets.Label(valuesMoodRect, moodText);
                ...
                //if (!ModSettings_Wages.fixedIncome)
                {
                    ...
                    for (int i = 0; i < moodSteps.Length; i++)
                    {
                        ...
                        {
                            GUI.color = Color.white;
                            Widgets.DrawLineVertical(baseOffsetLines + offsetThisLine, row.y, row.height);
                        }
                        /*else if (moodSteps[i].Value > 10)
                        {
                            GUI.color = Color.red;
